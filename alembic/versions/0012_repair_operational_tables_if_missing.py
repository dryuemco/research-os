"""repair operational tables when prior migrations were skipped

Revision ID: 0012_repair_operational_tables_if_missing
Revises: 0011_partner_intelligence_and_quality_foundation
...
revision = "0012_repair_operational_tables_if_missing"
down_revision = "0011_partner_intelligence_and_quality_foundation"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    if not _has_table("operational_job_configs"):
        op.create_table(
    ...
    if not _has_table("operational_job_runs"):
        op.create_table(
    ...
    if not _has_table("matching_runs"):
        op.create_table(
            "matching_runs",
...
def downgrade() -> None:
    # Safety migration: intentionally no destructive downgrade.
    pass
